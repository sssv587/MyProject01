...
void getCustomerInfo(struct Customer *customer) {
    printf("\n%d\t%s\t%c\t%d\t%s\t%s", customer->id, customer->name, customer->gender, customer->age, customer->phone,
           customer->email);
}
...
void listCustomers() {
    ...
    printf("---------------------------客户列表---------------------------");
    printf("\n编号\t姓名\t性别\t年龄\t电话\t邮箱");
    for (i = 0; i < customerNum; i++) {//表示客户编号
...
int del(int id) {
    ...
    int i;
    if (index == -1) {//说明不存在id这个客户
        return 0;//表示删除失败
    } else {
        //说明存在,就要删除,删除的本质就是将编号为id的这个客户移除
        //1.customers数组从index+1位置开始,整体前移
...
void main() {
    ...
    strcpy(customers[0].phone, "123456789");
    mainMenu();//调用函数显示菜单
}